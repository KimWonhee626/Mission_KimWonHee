...
public class Ut {
    ...
    public static class time{
        public static String diffTime(LocalDateTime time1, LocalDateTime time2) {
            String suffix = time1.isAfter(time2) ? "전" : "후";

            // 두개의 시간의 차이를 초로 환산
            long diff = Math.abs(ChronoUnit.SECONDS.between(time1, time2));

            long diffSeconds = diff % 60; // 초
            long diffMinutes = diff / (60) % 60; // 분
            long diffHours = diff / (60 * 60) % 24; // 시간
            long diffDays = diff / (60 * 60 * 24); // 일
